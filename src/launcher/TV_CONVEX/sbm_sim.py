        ...
        for b in np.logspace(0,2,5)[1:]:
            for l in [1, 2]:
                for pre in [True, False]:
                    methods.append({'name': 'tv_nc_beta{b:0>4d}_l{l:d}_pre{t}_sponge'.format(b=int(b),l=l,t=int(pre)),      'l_guess': 'sponge',            'is_unsupervised': True,  'method': TvNonConvex(num_classes=num_classes, verbosity=v, penalty_parameter=b, backtracking_tau_0=0.001, t_max_no_change=None, t_max=1e3)})
                    methods.append({'name': 'tv_nc_beta{b:0>4d}_l{l:d}_pre{t}_sncBNC'.format(b=int(b),l=l,t=int(pre)),      'l_guess': 'snc',               'is_unsupervised': False, 'method': TvNonConvex(num_classes=num_classes, verbosity=v, penalty_parameter=b, laplacian_scaling=l, run_pre_iteration=pre)})
                    methods.append({'name': 'tv_nc_beta{b:0>4d}_l{l:d}_pre{t}_sncSponge'.format(b=int(b), l=l, t=int(pre)), 'l_guess': 'sncSponge',         'is_unsupervised': False, 'method': TvNonConvex(num_classes=num_classes, verbosity=v, penalty_parameter=b, laplacian_scaling=l, run_pre_iteration=pre)})
